...
* 局部作用域
  * 块作用域
    * 只能在块内(包括函数块儿)使用的标识符具有块作用域
  * 文件作用域
    * 只能在当前源文件使用的标识符具有文件作用域
* 全局作用域
  * 在所有源文件内都能使用的标识符具有全局作用域
    ...
    * 比较运算
      ...
      * <, <=, > ,>=, ==, !=
        ...
        * 任意两个指针比较
          * ==，!=简单，不提了
          * <, <=, >, >=
            * 如果两个指针都指向同一个数组的元素或者数组最后一个元素的下一个元素，那么p<q表示，p指的元素在q指的元素之前,>同理
...
* return exit
  ...
  * return + 表达式
    ...
    * 表达式的值返回给上层函数或执行环境，用于函数之间通信。
    * 如果省略表达式，表示没有返回值，或者说返回值未定义
