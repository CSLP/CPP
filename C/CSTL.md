...
* 流(stream)是与磁盘或其他外围设备关联的数据的源或目的地。是C语言提供的抽象。
* 尽管在某些系统中(如UNIX),文本流和二进制流是相同的，但标准库仍然提供了这两种类型的流。
* 文本流是文本行组成的序列，每一行包含0个或多个字符，并以'\\n'结尾
  * 在某些环境中，可能需要将文本流转换为其他表现形式(如把'\\n'映射成回车和换行),或从其他表示形式转换为文本流
* 二进制流是有未经处理的字节构成的序列，这些字节记录着内部数据，并具有系列性质:
  * 如果在同一系统中写入二进制流，然后在读取该二进制流，则读出和写入的内容完全相同。
